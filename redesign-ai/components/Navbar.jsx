...
const SparkleIcon = ({ size = 20, className = "" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className}>
    <path d="M12 2C12 2 12.5 9.5 20 10C12.5 10.5 12 18 12 18C12 18 11.5 10.5 4 10C11.5 9.5 12 2 12 2Z" />
    <path d="M19 14C19 14 19.2 17.8 23 18C19.2 18.2 19 22 19 22C19 22 18.8 18.2 15 18C18.8 17.8 19 14 19 14Z" />
  </svg>
);
  ...
  return (
    <nav className={`navbar ${navbarIsDark ? 'scrolled shadow-lg bg-black/70 backdrop-blur-md' : 'bg-transparent'} fixed top-0 w-full z-[1000] flex justify-between items-center transition-all duration-300`}>
      ...
      <ul className={`nav-links fixed top-0 right-0 w-full h-dvh bg-black/95 flex flex-col transition-transform duration-500 ease-in-out z-[1050] ${isMobileMenuOpen ? 'translate-x-0 active' : 'translate-x-full'} md:static md:w-auto md:h-auto md:bg-transparent md:flex-row md:items-center md:gap-10 md:ml-auto md:transition-none md:translate-x-0`}>
        ...
        <li className="w-full md:w-auto px-8 md:px-0">
          <Link href="/ai-chat" onClick={closeMobileMenu} className={`block py-3 md:py-0 text-2xl md:text-lg uppercase tracking-wider transition-colors flex items-center gap-2 ${pathname === '/ai-chat' ? 'text-white drop-shadow-[0_0_10px_rgba(99,222,241,0.5)]' : 'text-gray-400 hover:text-white'}`}>
            <SparkleIcon size={18} className="text-[#00d2ff]" /> คุยกับ AI
          </Link>
        </li>
        <li className="w-full md:w-auto px-8 md:px-0">
